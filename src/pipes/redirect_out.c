...
int	last_red_in(t_node *temp, int red)
{
	...
	{
		if ((temp->type == RED_OUT || temp->type == APPEND) && red == 1)
			last_red = temp->pos;
		else if ((temp->type == RED_IN || temp->type == HEREDOC) && red == 0)
			last_red = temp->pos;
		temp = temp->next;
	}
	return (last_red);
...
int	redirect_out(t_node *current)
{
	...
	while (temp && temp->type != PIPE)
	{
		if (temp->pos == last_red && is_cm(current, -1))
		{
			if (temp->type == RED_OUT)
